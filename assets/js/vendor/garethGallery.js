...
GarethGallery.prototype.continueToLoad = function(){
    ...
    if(supports('transition')){
        ...
        if (this.options.transitionClass){  
            $(this.element).addClass(this.options.transitionClass);
            
            if(this.options.thumbnails){
                $('#'+this.options.thumbnails).addClass(this.options.transitionClass);
            }
        }
    }
...
GarethGallery.prototype.addTouchSupport = function(element, thumbs){
    ...
    $(element).bind('touchstart', function(event){
        ...
        /*if(event.data.instance.css3){
            if (event.data.instance.options.transitionClass){  
                $(event.data.instance.element).removeClass(event.data.instance.options.transitionClass);

                if(event.data.instance.options.thumbnails){
                    $('#'+event.data.instance.options.thumbnails).removeClass(event.data.instance.options.transitionClass);
                }
            }
        }*/
    ...
    $(element).bind('touchend', {instance:this}, function(event){
        ...
        if (touchCompletionTime < 1200){
            if(Math.abs(startPosition - endPosition) > 5){
                fastSwipe = true;
            }
        }
        // work out if move grater than threshold, and in which direction
        if ( (Math.abs(startPosition - endPosition) > ( event.data.instance.paneWidth / 2 ) ) || fastSwipe){
        ...
        /*if(event.data.instance.css3){
            if (event.data.instance.options.transitionClass){  
                $(event.data.instance.element).addClass(event.data.instance.options.transitionClass);

                if(event.data.instance.options.thumbnails){
                    $('#'+event.data.instance.options.thumbnails).addClass(event.data.instance.options.transitionClass);
                }
            }
        }*/
    });
...
var supports = (function(prop) {  
    ...
    if ( propCamel in div.style ) return true;
...
/* Utils is touch devcie function */
